    ...
    const fetchlatest = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL_ECOSOCH}/api/auth/get/LatestEvents/${_ID}`
        );
        setlatest(res.data);
    ...
    <Fragment>
      ...
      <Container fluid style={{ width: "85%" }} className="ptb--50">
        ...
        <div className="row">
          ...
          <div className="col-4">
            <img
              src={`${process.env.REACT_APP_API_URL_ECOSOCH}/${latest.EventsImage}`}
              alt=""
            />
          </div>
        </div>
...
export default LatestEventDesc;
